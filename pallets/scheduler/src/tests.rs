...
fn timelock_basic_scheduling_works() {
	new_test_ext().execute_with(|| {
		let ciphertext = vec![0];
		let bounded_ct: Ciphertext = Ciphertext::truncate_from(ciphertext);
		// Schedule call to be executed at the 4th block
		assert_ok!(Scheduler::do_schedule_sealed(DispatchTime::At(4), 127, root(), bounded_ct));
		// `log` runtime call should not have executed yet
		run_to_block(3);
		assert!(logger::log().is_empty());
		run_to_block(4);

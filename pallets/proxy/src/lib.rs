...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn proxy(
			...
			let def = Self::find_proxy(&real, Some(who), force_proxy_type)?;
		...
		/// - `delay`: The announcement period required of the initial proxy. Will generally be
		/// zero.
		/// - `anonymous`: Whether the account should be anonymous. If set to `true`, there is no
		/// delegate set.
		///
		...
		pub fn announce(
			...
			Proxies::<T>::get(&real)
				.0
				.into_iter()
				.find(|x| x.delegate == Some(who.clone()))
		...
		pub fn proxy_announced(
			...
			let delegate = T::Lookup::lookup(delegate)?;
			let real = T::Lookup::lookup(real)?;
			let def = Self::find_proxy(&real, Some(delegate.clone()), force_proxy_type)?;

...
pub trait Verifier {
	...
	fn verify(serialized_public_key: &[u8], pulse: Pulse) -> Result<bool, String>;
}
...
impl Verifier for QuicknetVerifier {
	fn verify(mut serialized_pk: &[u8], pulse: Pulse) -> Result<bool, String> {
		// decode public key (pk)
		let pk =
			ArkScale::<G2AffineOpt>::decode(&mut serialized_pk)
...
/// A verify to check pulses received from the Ideal Network.
// pub struct IDNVerifier;
// impl Verifier for IDNVerifier {
...
impl Verifier for UnsafeSkipVerifier {
	fn verify(_pk: &[u8], _pulse: Pulse) -> Result<bool, String> {
		Ok(true)
	}
}

...
pub type OpaquePublicKey = BoundedVec<u8, ConstU32<144>>;
/// an opaque hash type
...
impl BeaconInfoResponse {
	...
	pub fn try_into_beacon_config(&self) -> Result<BeaconConfiguration, String> {
		...
		let mut bounded_scheme_id = BoundedHash::try_from(self.scheme_id.as_bytes().to_vec().clone())
...
impl DrandResponseBody {
	pub fn try_into_pulse(&self) -> Result<Pulse, String> {
		...
		let bounded_signature = BoundedVec::<u8, ConstU32<48>>::try_from(self.signature.clone())
